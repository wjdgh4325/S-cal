...
parser.add_argument('--dist', type=str, default='lognormal', choices=['gamma', 'lognormal', 'weibull'])
...
if args.dist=='lognormal': 
    ...
    lambda_tr = p_surv_t.scale[ : VAL_SPLIT]
    k_tr = p_surv_t.concentration[ : VAL_SPLIT]
    lambda_va = p_surv_t.scale[VAL_SPLIT : TEST_SPLIT]
    k_va = p_surv_t.concentration[VAL_SPLIT : TEST_SPLIT]
    lambda_te = p_surv_t.scale[TEST_SPLIT : ]
    k_te = p_surv_t.concentration[TEST_SPLIT : ]

    dist_tr = torch.distributions.Weibull(lambda_tr, k_tr)
    dist_va = torch.distributions.Weibull(lambda_va, k_va)
    dist_te = torch.distributions.Weibull(lambda_te, k_te)

    torch.save(lambda_tr, args.dist + '_lambda_tr.pt')
    torch.save(k_tr, args.dist + '_k_tr.pt')
    torch.save(lambda_va, args.dist + '_lambda_va.pt')
    torch.save(k_va, args.dist + '_k_va.pt')
    torch.save(lambda_te, args.dist + '_lambda_te.pt')
    torch.save(k_te, args.dist + '_k_te.pt')
