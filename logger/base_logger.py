...
class BaseLogger(object):
    def __init__(self, args, dataset_len):
        ...
        log_dir = os.path.join('logs', args.name + '_bs' + str(args.batch_size) + \
                                '_ds_' + args.dataset + '_lam' + str(args.lam) + '_' + datetime.now().strftime('%b%d_%H%M'))
        self.summary_writer = SummaryWriter(log_dir = log_dir)
        self.epoch = args.start_epoch
        # Current iteration in epoch (i.e. # examples seen in the current epoch)
        self.iter = 0
        # Current iteration overall (i.e. total # of examplese seen)
